...
from aws_glue_workflow_analyzer.output import save_to_json, save_to_csv
...
def test_save_to_json_io_error(mock_logger, mock_file, sample_data):
    ...
    mock_logger.error.assert_called_once_with("Failed to save analysis results to JSON: Failed to write to file")
...
def test_save_to_csv(mock_logger, mock_file, sample_data):
    ...
    for item in sample_data:
        handle.write.assert_any_call(",".join(str(value) for value in item.values()) + "\r\n")
...
def test_save_to_csv_io_error(mock_logger, mock_file, sample_data):
    ...
    mock_logger.error.assert_called_once_with("Failed to save analysis results to CSV: Failed to write to file")
