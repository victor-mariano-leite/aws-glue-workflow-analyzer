...
def test_main_json_output(mock_console, mock_save_to_json, mock_analyzer, mock_parse_args):
    mock_parse_args.return_value = MagicMock(workflows=["workflow1"], days=30, output="output.json", format="json")
...
@patch("aws_glue_workflow_analyzer.__main__.console")
def test_main_csv_output(mock_console, mock_save_to_csv, mock_analyzer, mock_parse_args):
    mock_parse_args.return_value = MagicMock(workflows=["workflow1"], days=30, output="output.csv", format="csv")
    mock_analyzer_instance = MagicMock()
    mock_analyzer.return_value = mock_analyzer_instance
    mock_analyzer_instance.analyze_workflows.return_value = [{"key": "value"}]
...
def test_main_console_output(mock_console, mock_analyzer, mock_parse_args):
    mock_parse_args.return_value = MagicMock(workflows=["workflow1"], days=30, output=None, format="json")
    mock_analyzer_instance = MagicMock()
    mock_analyzer.return_value = mock_analyzer_instance
    mock_analyzer_instance.analyze_workflows.return_value = [{"key": "value"}]
...
def test_main_workflow_analyzer_error(mock_logger, mock_analyzer, mock_parse_args):
    ...
    mock_analyzer_instance = MagicMock()
    mock_analyzer.return_value = mock_analyzer_instance
    mock_analyzer_instance.analyze_workflows.side_effect = WorkflowAnalyzerError("Test error")

    main()

    mock_logger.error.assert_called_once_with("An error occurred during workflow analysis: Test error")

@patch("aws_glue_workflow_analyzer.__main__.parse_args")
@patch("aws_glue_workflow_analyzer.__main__.GlueWorkflowAnalyzer")
@patch("aws_glue_workflow_analyzer.__main__.logger")
def test_main_generic_exception(mock_logger, mock_analyzer, mock_parse_args):
    mock_parse_args.return_value = MagicMock(workflows=["workflow1"], days=30, output=None, format="json")
    mock_analyzer_instance = MagicMock()
    mock_analyzer.return_value = mock_analyzer_instance
    mock_analyzer_instance.analyze_workflows.side_effect = Exception("Generic error")

    main()

    mock_logger.error.assert_called_once_with("An unexpected error occurred: Generic error")
